...
describe 'Create tests for a hash' do
  ...
  small_fruit_nutrition_details = {
    ...
    it 'plums and apples should get no calories from fat' do
      expect(small_fruit_nutrition_details[:apples][:plums][:calories_from_fat]).to be 0
    end

    it 'each fruit should have 4 keys' do
      expect(small_fruit_nutrition_details).keys.lenght.to be eq 4
      
      small_fruit_nutrition_details.each do |k,v|
        expect(v.keys.lengh).to eq 4
    end

    it 'all calories should be between 40 to 150' do
      small_fruit_nutrition_details.each do |k,v|
        expect(v[:calories]).to be_between(40,150)
    # expect(small_fruit_nutrition_details).while 40 <>150
    end
  end
end